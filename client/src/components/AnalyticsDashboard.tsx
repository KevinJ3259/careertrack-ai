...
const statusOrder: Status[] = ["SAVED", "APPLIED", "INTERVIEW", "OFFER", "REJECTED"];
const chartColors = ["#2563eb", "#7c3aed", "#f59e0b", "#16a34a", "#dc2626"];
...
export default function AnalyticsDashboard({
  ...
  const totals = useMemo(() => {
    const submitted = applications.filter((application) =>
      ["APPLIED", "INTERVIEW", "OFFER", "REJECTED"].includes(application.status)
    ).length;
    ...
    return {
      ...
      submitted,
      interviewRate: submitted === 0 ? 0 : Math.round((interviews / submitted) * 100),
      offerRate: interviews === 0 ? 0 : Math.round((offers / interviews) * 100)
    };
  }, [applications]);
            ...
            <div className="analytics-chart">
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={monthlyData}>
                  ...
                  <Bar dataKey="interviews" name="Interview stage" fill="#f59e0b" radius={[6, 6, 0, 0]} />
                  <Bar dataKey="offers" name="Offers" fill="#16a34a" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </article>
